    ...
    <div className={`app-shell scene-${activeScene}`}>
      ...
      <header className="site-header">
        ...
        <nav className={`desktop-nav ${menuOpen ? "open" : ""}`} aria-label="Main navigation">
          {[["ABOUT", "#about"], ["DOMAINS", "#domains"], ["PRIZES", "#prizes"], ["TRANSPORT", "#transport"], ["COMMUNITY", "/community"], ["PARTNERS", "#sponsors"], ["FACULTY & TEAM", "#team"], ["FAQ", "#faq"]].map(([label, href]) => (href.startsWith("/") ? <Link key={label} href={href} onClick={closeMenu}>{label}</Link> : <a key={label} href={href} onClick={closeMenu}>{label}</a>))}
        </nav>
      ...
      <main id="top" className="page-content">
        ...
        <section id="team" className="team-section section-shell content-section scroll-scene" data-scene="team"><div className="section-rail"><span>08</span><i /><span>FACULTY & TEAM</span></div><div className="section-heading"><div className="eyebrow"><UsersRound size={15} /> FACULTY &amp; DEPARTMENT LEADERSHIP</div><h2>FACULTY &amp;<br /><i>COORDINATORS.</i></h2></div><div className="team-console-head"><p>Official Faculty Coordinators, Heads of Department, and Student Leadership for Mechanical Engineering, Robotics &amp; Automation, and Electronics &amp; Instrumentation Engineering.</p></div><div className="leadership-department-grid">{leadershipDepartments.map((department) => <LeadershipDepartmentCard department={department} key={department.department} />)}</div></section>
