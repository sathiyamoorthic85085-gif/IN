...
const row: RegistrationExportRow = { referenceCode: "IH26-TEST", teamName: "Test Squad", leadName: "Test Lead", email: "lead@example.com", phone: "+919999999999", college: "ESEC", memberOne: "One", memberTwo: "Two", memberThree: null, memberFour: null, memberCount: 2, domain: "Open Innovation", buildType: "software", transactionId: "UTR-123456", paymentStatus: "payment_pending", createdAt: new Date("2026-09-01T00:00:00Z") };

describe("registration workbook", () => {
  it("creates all eight domain sheets for each build type without browser downloads", () => {
    const workbook = createRegistrationWorkbook({ software: [row], hardware: [{ ...row, referenceCode: "IH26-HW", buildType: "hardware" }] });
  ...
  it("preserves the exact entered member, payment, and timestamp values in the matching sheet", () => {
    ...
    const [exported] = XLSX.utils.sheet_to_json<Record<string, unknown>>(workbook.Sheets["Open Innovation - Software"]);
  ...
  it("creates readable CSV for all registrations and scoped build types", () => {
    const hardwareRow = { ...row, referenceCode: "IH26-HW", buildType: "hardware" as const, memberCount: 3, memberThree: "Three" };
